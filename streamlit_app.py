...
st.markdown('<h1 style="text-align:center; margin-bottom: 30px;">SEC 10-K ➜ Year 0 ➜ DCF Model</h1>', unsafe_allow_html=True)
...
def fetch_text(url):
    try: return requests.get(url, headers={'User-Agent':'Mozilla/5.0'}).text
    except: return ""

def extract_year_0(txt):
    def find(p):
        m = re.search(p + r'[^>]*>\s*\(?([0-9]{1,3}(?:,[0-9]{3})*)\)?', txt, re.I)
        if m:
            v = float(m.group(1).replace(',', ''))
            if '(' in m.group(0) and ')' in m.group(0): v = -v
            return v / 1000 # Millions -> Billions
        return 0.0
    return {
        'Revenue': find(r'Revenues?|Net\s?Sales|Total\s?Revenues?'),
        'EBIT': find(r'Operating\s?Income|Operating\s?Profit'),
        'Depreciation': find(r'Depreciation\s?and\s?amortization'),
        'Capex': abs(find(r'Capital\s?expenditures|Additions\s?to\s?property')),
        'Debt': find(r'Total\s?debt|Long-term\s?debt'),
        'Cash': find(r'Cash\s?and\s?cash\s?equivalents')
    }
...
if url:
    with st.spinner("Analyzing Financials..."):
        ...
        if txt:
            ...
            if d['Revenue'] > 0: st.session_state.y0 = d
...
# 1. ALWAYS VISIBLE TABLE
st.subheader("Projected Free Cash Flow (Billions)")
st.dataframe(df.T.style.format("{:,.2f}"), use_container_width=True)

st.markdown("<br>", unsafe_allow_html=True)

# 2. VALUATION CARDS & BRIDGES (2 Columns)
col_g, col_e = st.columns(2)

# --- LEFT COLUMN: GORDON GROWTH ---
with col_g:
    # The Card
    st.markdown(f"""
    <div class="val-card border-purple">
        <div class="val-title">Perpetuity Growth <span style="float:right">🌊</span></div>
        <div class="val-sub">Valuation based on long-term growth (g)</div>
        <div class="val-label">IMPLIED SHARE PRICE</div>
        <div class="val-price text-purple">${p_g:.2f}</div>
        <div class="val-ev">
            <span>Enterprise Value</span>
            <strong>${ev_g:.2f}B</strong>
        </div>
    </div>
    """, unsafe_allow_html=True)
    
    # The Bridge
    st.markdown("##### Valuation Bridge (Gordon)")
    bridge_g = pd.DataFrame({
        "Component": ["PV of 5y Cash Flows", "PV of Terminal Value", "Enterprise Value", "Less: Net Debt", "Equity Value"],
        "Value": [sum_pv, pv_tv_g, ev_g, debt_in-cash_in, ev_g-(debt_in-cash_in)]
    }).set_index("Component")
    st.dataframe(bridge_g.style.format("${:,.2f}B"), use_container_width=True)

# --- RIGHT COLUMN: EXIT MULTIPLE ---
with col_e:
    # The Card
    st.markdown(f"""
    <div class="val-card border-green">
        <div class="val-title">Exit Multiple <span style="float:right">💼</span></div>
        <div class="val-sub">Valuation based on EBITDA multiple</div>
        <div class="val-label">IMPLIED SHARE PRICE</div>
        <div class="val-price text-green">${p_e:.2f}</div>
        <div class="val-ev">
            <span>Enterprise Value</span>
            <strong>${ev_e:.2f}B</strong>
        </div>
    </div>
    """, unsafe_allow_html=True)
    
    # The Bridge
    st.markdown("##### Valuation Bridge (Multiple)")
    bridge_e = pd.DataFrame({
        "Component": ["PV of 5y Cash Flows", "PV of Terminal Value", "Enterprise Value", "Less: Net Debt", "Equity Value"],
        "Value": [sum_pv, pv_tv_e, ev_e, debt_in-cash_in, ev_e-(debt_in-cash_in)]
    }).set_index("Component")
