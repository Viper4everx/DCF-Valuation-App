...
with st.sidebar:
    ...
    # Determine Multipliers based on Scenario
    if "Bull" in scenario:
    ...
    # WACC is now constant across scenarios
    ...
    # Apply Scenario Multipliers to Defaults
...
# ==========================================
# Isolated function for Sensitivity Matrix re-use
def calculate_dcf_value(r_0, e_0, d_0, c_0, g, m, t, w, l, em, debt, cash, shares, years=5):
    # Safe checks
    if w <= l: l = w - 0.005 # Prevent div/0
...
try:
    ...
    # Re-calculate based on Edited Table
    for y in years_range:
        col_name = f"Year {y}"
...
for t_g in ltg_range:
    ...
    for w_r in wacc_range:
        # Use our isolated calculation function
...
# Styling the Heatmap (Kept your preferred glass theme colors)
def style_sens(val):
    color = '#2a2a3e' # default
    if val > cur_price * 1.1: color = '#105234' # Deep Green
    elif val < cur_price * 0.9: color = '#4a151b' # Deep Red
